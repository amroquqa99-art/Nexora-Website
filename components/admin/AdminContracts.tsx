...
const AdminContracts = () => {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin_contracts"] }); setEditing(null); toast({ title: "✓" }); },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin_contracts"] }); toast({ title: "✓" }); },
  });

  const statusColors: Record<string, string> = {
    draft: "text-muted-foreground bg-muted", sent: "text-blue-400 bg-blue-500/10",
    signed: "text-green-400 bg-green-500/10", expired: "text-red-400 bg-red-500/10",
  };
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setEditing({ client_id: "", project_id: null, title: "", content: "", status: "draft" })}
          className="flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90"><Plus className="w-4 h-4" /> New Contract</button>
      </div>

      {editing && (
        <div className="glass-card p-6 mb-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-semibold text-foreground">{editing.id ? "Edit" : "New"} Contract</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <input placeholder="Contract Title" value={editing.title || ""} onChange={e => setEditing({ ...editing, title: e.target.value })} className="px-3 py-2 rounded-lg bg-muted border border-border text-foreground text-sm md:col-span-2" />
          ...
          <textarea placeholder="Contract Content..." value={editing.content || ""} onChange={e => setEditing({ ...editing, content: e.target.value })}
            className="w-full mt-4 px-3 py-2 rounded-lg bg-muted border border-border text-foreground text-sm resize-none" rows={8} />
          ...
          <div key={c.id} className="glass-card p-4 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">{c.profiles?.full_name}{c.client_projects?.title ? ` · ${c.client_projects.title}` : ""}</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <span className={`text-xs px-2 py-0.5 rounded-full ${statusColors[c.status] || ""}`}>{c.status}</span>
              {c.signed_at && <span className="text-[10px] text-muted-foreground">{new Date(c.signed_at).toLocaleDateString()}</span>}
              <button onClick={() => setEditing({ ...c })} className="p-2 hover:bg-muted rounded-lg"><Pencil className="w-4 h-4 text-muted-foreground" /></button>
              <button onClick={() => { if (confirm("Delete?")) deleteMutation.mutate(c.id); }} className="p-2 hover:bg-destructive/10 rounded-lg"><Trash2 className="w-4 h-4 text-destructive" /></button>
            </div>
          </div>
        ))}
      </div>
